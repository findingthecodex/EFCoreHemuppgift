...
public class Program
{
   public static async Task Main(string[] args)
   {
      ...
      using (var db = new ShopContext())
      {
         ...
         if (!await db.Customers.AnyAsync())
         {
            db.Customers.AddRange(
               new Customer
               {
                  CustomerId = 1, CustomerName = "Amanda Edenå", CustomerEmail = "amanda.edenå@example.com",
                  CustomerCity = "Stockholm"
               },
               new Customer
               {
                  CustomerId = 2, CustomerName = "Björn Svensson", CustomerEmail = "bjorn.svensson@example.com",
                  CustomerCity = "Göteborg"
               },
               new Customer
               {
                  CustomerId = 3, CustomerName = "Carina Larsson", CustomerEmail = "carina.larsson@example.com",
                  CustomerCity = "Malmö"
               }
            );
      ...
      static async Task OrderMenu()
      {
         ...
         {
            Console.WriteLine(
               "\nOrder Menu: 1. Order-List | 2. Order-Details | 3. New-Order | 4. Status | 5. Exit");
            Console.WriteLine(">");
            var line = Console.ReadLine()?.Trim() ?? string.Empty;
